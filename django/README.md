...
**Soft bans (не возвращать):** WebSocket Messenger / `consumers.py` / `CHANNEL_LAYERS` / `static/js/messenger.js`, PYMK, Ads/Flyers/Beacon/Connect, **third-party** app platform (внешний canvas/хостинг чужого кода — нельзя; first-party Platform-приложения ВДрузья под chrome 2006 — можно), отдельный video-CDN продукт (видео — на том же медиа-диске/S3, что и фото), Facebook Home, blue Timeline redesign (`page-tabs` / flex-hero), `models/polls.py` (использовать только `ClassicPoll`).
