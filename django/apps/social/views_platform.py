    ...
    if q and not tool:
        ...
        for topic, items in topics.items():
            ...
            if hit:
                filtered[topic] = hit
        topics = filtered
    from apps.social.calculators import copy as calc_copy
    law_note = ""
    if tool and tool.get("law") == "vat":
        law_note = calc_copy.VAT_NOTE
    elif tool and tool.get("law") == "ndfl":
        law_note = calc_copy.NDFL_NOTE
    elif tool and tool.get("law") == "contrib":
        law_note = calc_copy.CONTRIB_NOTE
    if tool:
        raw = {}
        if request.method == "POST":
...
def _canvas_chess(request, me, app):
    ...
    from apps.social.chess import lessons as chess_lessons
    from apps.social.chess import service as chess
    from apps.social.chess.models import ChessGame, ChessMove

    tab = (request.GET.get("tab") or request.POST.get("tab") or "play").strip()
    if tab not in ("play", "game", "stats", "ratings", "champs", "learn"):
        tab = "play"
    friends = pa.friends_for_app(me)
    ...
    if request.method == "POST":
        ...
        try:
            if action == "start":
                fid = int(request.POST.get("friend_id") or 0)
                color = (request.POST.get("color") or "white").strip()
                ...
                if color == "black":
                    g = chess.start_game(peer, me, in_champ=True)
                else:
                    g = chess.start_game(me, peer, in_champ=True)
                notify.push(
                    peer.id,
                    title="Шахматы",
            ...
            if action == "move":
                ...
                g = chess.game_for(me, gid)
                if not g:
                    raise ValueError("Партия не найдена")
                frm = (request.POST.get("from_sq") or "").strip()
                to = (request.POST.get("to_sq") or "").strip()
                g = chess.play_move(g, me, frm, to)
                opp_id = g.black_id if me.id == g.white_id else g.white_id
                notify.push(
                    opp_id,
                    title="Шахматы",
                    body=f"{me.name}: {frm}-{to}",
                    type="message",
                    url=reverse("apps.canvas", args=["chess"]) + f"?tab=game&id={g.id}",
                )
                if g.result != "*":
                    ...
                    messages.success(request, f"Ход {frm}-{to} сделан.")
                return redirect(reverse("apps.canvas", args=["chess"]) + f"?tab=game&id={g.id}")
            if action == "resign":
                gid = int(request.POST.get("game_id") or 0)
                g = chess.game_for(me, gid)
            ...
            messages.error(request, "Не удалось выполнить действие. Проверьте ход.")
    ...
    if tab == "game":
        ...
        if game:
            ...
            # optional legal targets if from_sq in GET
            frm = (request.GET.get("from") or "").strip()
            if frm and game.result == "*" and chess.side_of(game, me) == game.turn:
                try:
                    st = chess_engine.parse_fen(game.fen)
                    legal_hint = chess_engine.legal_moves_from(st, frm)
                except Exception:
                    legal_hint = []
        else:
    ...
    ratings = chess.leaderboard(25) if tab == "ratings" else None
    ...
    if tab == "learn":
        ...
        lesson_list = chess_lessons.LESSONS
    else:
        lesson_list = []
    ...
    return render(request, "social/apps/canvas_chess.html", {
        ...
        "legal_hint": legal_hint, "from_sq": (request.GET.get("from") or ""),
        "stats": stats, "ratings": ratings,
        "standings": standings, "past_champs": past_champs,
        "lesson": lesson, "lesson_list": lesson_list,
